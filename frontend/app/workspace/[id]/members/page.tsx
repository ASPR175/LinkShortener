...
const updateRole = async (memberId: string, role: "member" | "admin") => {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/workspace/${workspaceId}/members/${memberId}`,
      {
        method: "PATCH",
        headers: {
...
const removeMember = async (memberId: string) => {
  ...
  try {
    ...
    if (res.ok) setMembers(prev => prev.filter(m => m._id !== memberId));
  } catch (err) {
    console.error(err);
  }
};
                ...
                {m.role !== "owner" && (
                  <>
                    <button
                      ...
                      onClick={() =>
                        updateRole(m._id, m.role === "admin" ? "member" : "admin")
                      }
                    >
